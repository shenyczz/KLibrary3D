...
namespace DX12
{
	...
	void MeshFactory::CreateTriangle(const float side, MeshData* pMeshData)
	{
		...
		*/
		...
		for (int i = 0; i < 3; i++)
		{
			...
			pMeshData->Vertices.push_back(Vertex(pnts[i], noms[i], tags[i], texs[i], clrs[i]));
		}
	...
	void MeshFactory::CreateQuad(float x, float y, float w, float h, float depth, MeshData* pMeshData)
	{
		...
		//pMeshData->Indices32[0] = 0;
		//pMeshData->Indices32[1] = 1;
		//pMeshData->Indices32[2] = 2;

		//pMeshData->Indices32[3] = 0;
		//pMeshData->Indices32[4] = 2;
		//pMeshData->Indices32[5] = 3;
	...
	void MeshFactory::CreateGrid(float width, float depth, uint32 m, uint32 n, MeshData* pMeshData)
	{
		...
		for (uint32 i = 0; i < m; ++i)
		{
			...
			for (uint32 j = 0; j < n; ++j)
			{
				...
				pMeshData->Vertices[i*n + j].TexC.x = j * du;
				pMeshData->Vertices[i*n + j].TexC.y = i * dv;
			}
		}
	...
	void MeshFactory::CreateGeosphere(float radius, uint32 numSubdivisions, MeshData* pMeshData)
	{
		...
		for (uint32 i = 0; i < pMeshData->Vertices.size(); ++i)
		{
			...
			pMeshData->Vertices[i].TexC.x = theta / XM_2PI;
			pMeshData->Vertices[i].TexC.y = phi / XM_PI;
	...
	//static
	void MeshFactory::CreateCylinder(float btmRadius, float topRadius, float height,
...
#pragma region --Private Functions
	...
	//static
	void MeshFactory::BuildCylinderTopCap(float bottomRadius, float topRadius, float height,
	...
	//static
	void MeshFactory::BuildCylinderBottomCap(float bottomRadius, float topRadius, float height,
		uint32 sliceCount, uint32 stackCount, MeshData& meshData)
	{
		// 
		// Build bottom cap.
