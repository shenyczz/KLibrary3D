...
namespace DX12
{
	...
	void MeshFactory::CreateTriangle(const float side, MeshData* pMesh)
	{
		...
		for (int i = 0; i < 3; i++)
		{
			...
			pMesh->AddVertex(Vertex(pnts[i], noms[i], tags[i], texs[i], clrs[i]));
		}
	...
	void MeshFactory::CreateQuad(float width, float height, MeshData* pMesh)
	{
		...
		for (int i = 0; i < 4; i++)
		{
			...
			pMesh->AddVertex(Vertex(pnt, noms[i], tags[i], texs[i], clrs[i]));
		}
	...
	void MeshFactory::CreateBox(float width, float height, float depth, MeshData* pMesh)
	{
		...
		// 顶点数据
		float factor = 0.5f;
		float w = factor * width;
		float h = factor * height;
		float d = factor * depth;
		...
		for (size_t i = 0; i < pnts.size(); i++)
		{
			...
			pMesh->AddVertex(Vertex(pnt, clrs[i]));
		}

		// 为了能看见正前面
		pMesh->OffsetPositionZ(factor * depth);
