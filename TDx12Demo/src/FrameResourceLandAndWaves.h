...
struct __FrameResourceLaw : public FrameResourceBase
{
	__FrameResourceLaw(ID3D12Device* device)
...
struct FrameResourceLaw : public __FrameResourceLaw
{
		...
		, WavesVBs(nullptr)
	{
		...
		WavesVBs = std::make_unique<TUploadBuffer<Vertex>>(device, waveVertCount, false);
	}
	...
	std::unique_ptr<TUploadBuffer<Vertex>> WavesVBs;


	int PassCount() { return m_PassCount; }
	int ObjectCount() { return m_ObjectCount; }
	int WaveVertexCount() { return m_waveVertexCount; }

private:
	int m_PassCount;		// 渲染过程数量
	int m_ObjectCount;		// 渲染对象数量
	int m_waveVertexCount;	// 波浪顶点数量
